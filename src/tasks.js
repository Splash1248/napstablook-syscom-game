export const FIGHT_TASKS = [
  ...
  },
  {
    ...
    steps: [
      { cmd: "fullscan 10.0.0.66", output: "Filtered ports bypassed. Exposed service 'ghoul-rpc'. Furthur inspection needed." },
  ...
  {
    id: "fight_3",
    prompt: "Unencrypted authentication traffic detected from host 10.0.0.88. Intercept the live connection to capture credentials, then exploit with the extracted token on the vulnerable port.",
    steps: [
      { cmd: "intercept 10.0.0.88", output: "Packet captured [SRC: 10.0.0.88 DST: 10.0.0.1]. Payload loaded: 'AUTH_KEY=ghost_root_9921'. Service on port 22" },
      { cmd: "exploit 10.0.0.88 22", output: "[SUCCESS] Token accepted on SSH port 22. Napstablook loses 25% HP!" }
    ]
  },
  {
    id: "fight_4",
    prompt: "Napstablook is routing traffic through gateway 10.0.0.1 from target host 10.0.0.99. Trace the path to identify the gateway and host, poison the ARP routing table between the gateway and host, then deploy an exploit against the intercepted session.",
    steps: [
      { cmd: "tracepath 10.0.0.99", output: "Hop 1: 10.0.0.1 (Gateway) -> Hop 2: 10.0.0.99 (Target). Route unencrypted." },
      { cmd: "mitm 10.0.0.1 10.0.0.99", output: "[ARP SPOOF SUCCESS] Traffic redirected through local interface. Port 9000 exposed on 10.0.0.99." },
      { cmd: "exploit 10.0.0.99 9000", output: "[PWNED] Route hijacked and session dropped. Napstablook loses 25% HP!" }
    ]
  }
];
...
export const ACT_TASKS = [
  ...
  },
  {
    ...
    steps: [
      { cmd: "scale compliments 5", output: "[WARNING] Daemon rejected connection: 'oh... i'd just weigh you down...'" }
    ]
  },
  {
    id: "threat_1",
    ...
    steps: [
      { cmd: "rollback panic-engine", output: "[WARNING] Daemon rejected connection: 'go ahead, do it...'" }
    ]
  },
  {
    id: "cheer_1",
    ...
    steps: [
      { cmd: "readlogs dapper-blook", output: "[LOGS] Hat constructed: SUCCESS | Melancholy down 10% | Progress: [1 CHEER IS RECORDED]" }
    ]
  },
  {
    id: "cheer_2",
    ...
    steps: [
      { cmd: "scale smile-daemon 3", output: "[SCALED] smile-daemon instances updated to 3. Napstablook blushes slightly. Progress: [1 CHEER IS RECORDED]" }
    ]
  },
  {
    id: "cheer_3",
    ...
    steps: [
      { cmd: "syscheck", output: "[SYSINFO] ENV: MOOD=Encouraged, STYLE=DapperHatDeployed | Victory condition draws closer! [1 CHEER IS RECORDED]" }
    ]
  }
];
